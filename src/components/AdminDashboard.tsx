...
import { Shield, Flame, Zap, CheckCircle, XCircle, Trophy, Users, CreditCard, ArrowLeftRight, RefreshCw } from 'lucide-react';
import Header from './Header';
import { useApp } from '../contexts/AppContext';
import { supabase } from '../lib/supabase';
import { todayStr, currentMonthStr, nextMonthStr, formatMonthJP } from '../utils/date';
...
export default function AdminDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [{ data: mems }, { data: pays }, { data: reqs }, { data: recs }] = await Promise.all([
  ...
  useEffect(() => {
    const ensureMonthlyBilling = async () => {
      const { data: activeMembers } = await supabase.from('hf_members').select('*').eq('status', 'active');
      if (!activeMembers) return;
      for (const m of activeMembers) {
        const { data: existing } = await supabase.from('hf_payments').select('id')
          .eq('member_id', m.id).eq('type', 'monthly').eq('billing_month', month).maybeSingle();
        if (!existing) {
          await supabase.from('hf_payments').insert({ member_id: m.id, amount: getMonthlyCost(m), type: 'monthly', billing_month: month, status: 'pending' });
        }
      }
      ...
      <main className="max-w-md mx-auto px-5 pt-6 pb-16 space-y-5">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={fetchAll} disabled={isRefreshing} className="p-2 bg-slate-800 rounded-xl text-slate-400 hover:text-white transition-colors">
              <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            </button>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-800 rounded-2xl p-3 text-center">
              <p className="text-2xl font-black text-white">{submittedToday.size}<span className="text-slate-500 text-lg">/{activeMembers.length}</span></p>
            ...
            <div className="bg-slate-800 rounded-2xl p-3 text-center">
              <p className={`text-2xl font-black ${payments.length > 0 ? 'text-amber-400' : 'text-white'}`}>{payments.length}</p>
              ...
              {pendingMembers.map(m => {
                ...
                return (
                  <motion.div key={m.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {payment && <button onClick={() => confirmPayment(payment.id).then(fetchAll)}
                      className="flex-shrink-0 px-3 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-500 transition-colors">承認</button>}
                  </motion.div>
                );
              })}
            </div>
              ...
              {monthlyPayments.map(p => {
                ...
                return (
                  <motion.div key={p.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <button onClick={() => confirmPayment(p.id).then(fetchAll)}
                      className="flex-shrink-0 px-3 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-500 transition-colors">確認</button>
                  </motion.div>
                );
              })}
            </div>
              ...
              {planRequests.map(req => (
                <motion.div key={req.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-9 h-9 bg-slate-800 rounded-xl flex items-center justify-center text-lg flex-shrink-0">{req.member?.emoji}</div>
                    <div>
                      <p className="font-bold text-white text-sm">{req.member?.name}</p>
                      <p className="text-xs text-slate-400">{PLAN_LABELS[req.from_plan]} → {PLAN_LABELS[req.to_plan]}</p>
                      <p className="text-xs text-slate-500">{req.effective_month} 〜 適用</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => approvePlanChange(req.id).then(fetchAll)}
                    ...
                    <button onClick={() => rejectPlanChange(req.id).then(fetchAll)}
          ...
          <div className="space-y-3">
            {activeMembers.map((member, i) => (
              <motion.div key={member.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.05 }} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                <div className="p-4 flex items-start gap-3">
                  <div className="w-11 h-11 bg-slate-800 rounded-xl flex items-center justify-center text-xl flex-shrink-0">{member.emoji}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-bold text-white truncate">{member.name}</p>
                      {submittedToday.has(member.id)
                        ? <div className="flex items-center gap-1 flex-shrink-0"><CheckCircle className="w-4 h-4 text-emerald-400" /><span className="text-emerald-400 text-xs font-semibold">提出済</span></div>
                        : <div className="flex items-center gap-1 flex-shrink-0"><XCircle className="w-4 h-4 text-slate-600" /><span className="text-slate-600 text-xs font-semibold">未提出</span></div>}
                    </div>
                    <div className="flex items-center gap-3 mt-1.5">
                      <div className="flex items-center gap-1"><Flame className="w-3.5 h-3.5 text-orange-500" /><span className="text-orange-400 text-sm font-bold">{member.streak}日</span></div>
                      <div className="flex items-center gap-1"><Trophy className="w-3.5 h-3.5 text-yellow-500" /><span className="text-slate-400 text-xs">最大 {member.max_streak}日</span></div>
                    </div>
                ...
                <div className="border-t border-slate-800 px-4 py-3">
                  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.97 }}
                    onClick={() => applyPenalty(member.id).then(fetchAll)}
                    ...
                    <Zap className="w-4 h-4" />ストリークをリセット（ペナルティ）
                  </motion.button>
                </div>
              </motion.div>
            ))}
        ...
        <p className="text-center text-slate-600 text-xs px-4 pb-4 leading-relaxed">ペナルティを適用すると、そのメンバーのストリークが 0 にリセットされ、次回ログイン時に警告メッセージが表示されます。</p>
      </main>
    </motion.div>
  );
}
